...
{    
    ...
    boolean fillInFreeDirection(ArrayList<Coordinates> shipCases, ShipType type, ProbabilityGrid grid) {
        if (Coordinates.alignedX( (Coordinates[]) shipCases.toArray() )) {
            ...
            if (startY == 0 || grid.cases[x][startY - 1].getContents() != CaseContents.UNKNOWN) {
                // fill in cases from the start case to the end
                setCasesBetween(shipCases, type, grid);
                // then from the end to the full length of the ship type
                ArrayList<Coordinates> endCases = new ArrayList<>();
                endCases.add(new Coordinates(x, endY));
                // the end coordinate must be one over to give a correct fill
                endCases.add(new Coordinates(x, startY + type.getLength() ));
                return setCasesBetween(endCases, type, grid);
            }
            else if (endY == grid.getWidth() - 1 || grid.cases[x][endY + 1].getContents() != CaseContents.UNKNOWN) {
                // fill in cases from the start case to the end
                setCasesBetween(shipCases, type, grid);
                // then backwards from the start to make the full length of the ship type
                ArrayList<Coordinates> beforeCases = new ArrayList<>();
                beforeCases.add(new Coordinates(x, startY));
                beforeCases.add(new Coordinates(x, endY - type.getLength() ));
                return setCasesBetween(beforeCases, type, grid);
            }
            return false;
        }
        else if (Coordinates.alignedY( (Coordinates[]) shipCases.toArray() )) {
            ...
            if (startX == 0 || grid.cases[startX-1][y].getContents() != CaseContents.UNKNOWN) {
                ...
                // then from the end to the full length of the ship type
                ArrayList<Coordinates> endCases = new ArrayList<>();
                endCases.add(new Coordinates(endX, y));
                // the end coordinate must be one over to give a correct fill
                endCases.add(new Coordinates(startX + type.getLength(),y));
                return setCasesBetween(endCases, type, grid);
            }
                ...
                // then backwards from the start to make the full length of the ship type
                ArrayList<Coordinates> beforeCases = new ArrayList<>();
                beforeCases.add(new Coordinates(startX, y));
                beforeCases.add(new Coordinates(endX - type.getLength(), y ));
                return setCasesBetween(beforeCases, type, grid);
            }
            return false;
        }
    ...
    boolean inferForType(ShipType type, ArrayList<Coordinates> shipCases) {
         ...
         * logic as there are 2 of them on the grid. Deal with these separately.
         */
        if (type == ShipType.CRUISER) {
            return inferForCruisers(shipCases);
        }
